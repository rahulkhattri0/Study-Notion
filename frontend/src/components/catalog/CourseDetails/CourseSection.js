  ...
  return (
    <div className='flex flex-col border-2 border-richblack-100 rounded-lg mb-3'>
        <div className='bg-richblack-600 p-2 rounded-lg flex flex-row justify-between items-center'>
            <div className='flex flex-row gap-x-2 items-center cursor-pointer m-3' onClick={()=>setVisible((prev)=>!prev)}>
                {!visible ? (<FaAngleDown className='text-lg text-richblack-5'/>) : (<FaAngleUp className='text-lg text-richblack-5'/>) }
        ...
        <div className={`${!visible ? 'h-0' : 'h-[10px]'} transition-height ease-in-out duration-700`}></div>
        {
            visible && (
                <div className='m-3'>
                    {
